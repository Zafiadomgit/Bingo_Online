...
export function BingoCardComponent({
  ...
  // Validar que el card tenga las propiedades necesarias
  if (!card || !card.numbers || !Array.isArray(card.numbers) || !card.marked_positions || !Array.isArray(card.marked_positions)) {
    console.error('BingoCardComponent: Card inválido:', card)
    return (
      <Card className="p-4 w-fit">
        <div className="text-center text-red-500">
          <p>Error: Cartón inválido</p>
        </div>
      </Card>
    )
  }
  ...
  const isCellMarked = (position: number) => {
    if (position < 0 || position >= 25 || !localMarkedPositions || !card.numbers) return false
    return localMarkedPositions[position] || (card.numbers[position] !== 0 && isNumberCalled(card.numbers[position]))
  }

  const renderCell = (position: number) => {
    if (position < 0 || position >= 25 || !card.numbers) {
      return null
    }
