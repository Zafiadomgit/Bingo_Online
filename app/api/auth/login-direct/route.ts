...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user) {
      return NextResponse.json({ success: false, error: 'Usuario no registrado', details: 'Verifica tu email e intenta nuevamente.' }, { status: 401 })
    }

    const passwordMatch = await bcrypt.compare(password, user.password)
    if (!passwordMatch) {
      return NextResponse.json({ success: false, error: '❌ Credenciales inválidas', details: 'Verifica tu email y contraseña e intenta nuevamente.' }, { status: 401 })
    }
    ...
    console.error('Login direct error:', error)
