...
export async function POST(request: NextRequest) {
  try {
    ...
    if (game.status !== 'ACTIVE') return NextResponse.json({ success: false, error: 'El juego no está activo' }, { status: 400 })
    ...
    const updatedGames = await supabaseFetch(`bingo_games?id=eq.${gameId}`, { method: 'PATCH', body: JSON.stringify(updateData) })

    return NextResponse.json({ success: true, game: updatedGames?.[0], newNumber, number: newNumber, gameFinished: updateData.status === 'FINISHED' })
  } catch (error: any) {
