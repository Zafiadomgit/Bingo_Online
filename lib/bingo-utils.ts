...
export function checkBingoWin(
  ...
  winType: 'line' | 'two-lines' | 'full-card' = 'line'
): boolean {
  ...
  const totalLines = completedRows.length + completedCols.length + (diag1Complete ? 1 : 0) + (diag2Complete ? 1 : 0)

  // Verificar según el tipo de ganancia
  switch (winType) {
    case 'line':
      return totalLines >= 1
    case 'two-lines':
      return totalLines >= 2
    case 'full-card':
      // Verificar que TODAS las casillas estén marcadas (cartón lleno)
      return effectiveMarked.every(marked => marked === true)
    default:
      return totalLines >= 1
  }
}
